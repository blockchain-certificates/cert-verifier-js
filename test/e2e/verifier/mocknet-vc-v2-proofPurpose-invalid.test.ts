...
describe('given the proofPurpose of the certificate\'s proof does not match the verifier\'s purpose', function () {
  ...
  it('should fail verification', async function () {
    ...
    const certificate = new Certificate(MocknetVCV2ValidFromValid, { proofPurpose: 'authentication' });
    await certificate.init();
    const result = await certificate.verify();
    expect(result.status).toBe(VERIFICATION_STATUSES.FAILURE);
    expect(result.message).toBe('Invalid proof purpose. Expected authentication but received assertionMethod');
    vi.restoreAllMocks();
  });
});
