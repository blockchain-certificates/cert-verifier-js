...
export class CertificateVerifier {
   ...
   * @param {*} err 
   */
  _failed(stepCode, completionCallback, err) {
    log(`failure:${err.message}`);
    this.statusCallback(stepCode, err.message, Status.failure);
    completionCallback(stepCode, err.message, Status.failure);
  ...
  async verify(completionCallback) {
    ...
    try {
      ...
      if (e instanceof VerifierError) {
        return this._failed(Status.final, completionCallback, e);
      }
      throw e;
    }
  }
