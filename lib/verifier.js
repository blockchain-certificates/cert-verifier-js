...
import { Status, CertificateVersion, Contexts, Url } from '../config/default';

const log = debug("verifier");

require('string.prototype.startswith');

const { obi: OBI_CONTEXT, blockcerts: BLOCKCERTS_CONTEXT, blockcertsv2: BLOCKCERTSV2_CONTEXT } = Contexts;
const CONTEXTS = {};
// Preload contexts
...
var getTxUrlsAndParsers = function (transactionId, chain) {
  ...
  let blockrIoUrl;
  if (chain === bitcoin.networks.bitcoin) {
    blockrIoUrl = Url.blockrIoUrl + transactionId;
  } else {
    blockrIoUrl = Url.blockrIoTestUrl + transactionId;
  }
  let blockrIoHandler = {
    url: blockrIoUrl,
...
export class CertificateVerifier {
  ...
  _succeed(completionCallback) {
    log("success")
    return completionCallback(null, "success");
  }
