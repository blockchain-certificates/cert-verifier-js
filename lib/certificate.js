...
class Certificate {
  ...
  static parseV2(certificateJson) {
    ...
    const uid = certificateJson.id; // TODO: remove urn:uuid:?
    const issuer = badge.issuer;
    const receipt = certificateJson.signature.merkleProof;
    const signature = certificateJson.signature.signatureValue;
    ...
    return new Certificate(CertificateVersion.v2_0, name, title, subtitle, description, certificateImage, signatureImage, sealImage, uid,
      issuer, receipt, signature, publicKey);
  }
