...
import {Blockchain, CertificateVersion, Contexts} from '../config/default';
...
export function ensureNotRevokedBySpentOutput(revokedAddresses, issuerRevocationKey, recipientRevocationKey) {
  ...
  if (isRevokedByIssuer) {
    throw new VError("This certificate batch has been revoked by the issuer.");
  }
  const isRevokedByRecipient = (-1 != revokedAddresses.findIndex((address) => address === recipientRevocationKey));
  if (isRevokedByRecipient) {
    throw new VError("This recipient's certificate has been revoked.");
  }
}
...
export function ensureNotRevokedByList(revokedAssertions, assertionUid) {
  ...
  if (isRevokedByIssuer) {
    throw new VError("This certificate has been revoked by the issuer.");
  }
}
...
export function ensureIssuerSignature(issuerKey, certificateUid, certificateSignature, chain) {
  ...
  if (!bitcoin.message.verify(issuerKey, certificateSignature, certificateUid, bitcoinChain)) {
    throw new VError("Issuer key doesn't match derived address.");
  }
}

export function ensureHashesEqual(actual, expected) {
  if (actual !== expected) {
    throw new VError("Computed hash does not match remote hash");
  }
}

export function ensureMerkleRootEqual(merkleRoot, remoteHash) {
  if (merkleRoot !== remoteHash) {
    throw new VError("Merkle root does not match remote hash.");
  }
}
...
export function ensureValidIssuingKey(keyMap, txIssuingAddress, txTime) {
  ...
  if (!validKey) {
    throw new VError("Transaction occurred at time when issuing address was not considered valid.");
  }
};
    ...
    if (!!proof) {
      for (var index in proof) {
        ...
        if (typeof node.left !== "undefined") {
          ...
          throw new VError("We should never get here.");
        }
      }
    }
  } catch (e) {
    throw new VError("The receipt is malformed. There was a problem navigating the merkle tree in the receipt.");
  }

  if (proofHash !== merkleRoot) {
    throw new VError("Invalid Merkle Receipt. Proof hash didn't match Merkle root");
  }
};
...
export function computeLocalHash(document, version) {
  ...
  if (version === CertificateVersion.v2_0) {
    expandContext.push({"@vocab": "http://fallback.org/"});
  }
  var nodeDocumentLoader = jsonld.documentLoaders.node({request: request});
  ...
  return new Promise((resolve, reject) => {
    jsonld.normalize(theDocument, {
      ...
      if (!!err) {
        reject(new VError(err, "Failed JSON-LD normalization"));
      } else {
        let unmappedFields = getUnmappedFields(normalized);
        if (unmappedFields) {
          reject(new VError("Found unmapped fields during JSON-LD normalization: " + unmappedFields.join(",")));
        }
...
export function ensureNotExpired(expires) {
  ...
  if (new Date() >= expiryDate) {
    throw new VError("This certificate has expired.");
  }
  // otherwise, it's fine
}
