...
export function ensureNotRevokedBySpentOutput(revokedAddresses, issuerRevocationKey, recipientRevocationKey) {
  if (issuerRevocationKey) {
    ...
    if (isRevokedByIssuer) {
      throw new VerifierError(Status.checkingRevokedStatus, "This certificate batch has been revoked by the issuer.");
    }
  }
  if (recipientRevocationKey) {
    ...
    if (isRevokedByRecipient) {
      throw new VerifierError(Status.checkingRevokedStatus, "This recipient's certificate has been revoked.");
    }
  }
}
...
export function ensureNotRevokedByList(revokedAssertions, assertionUid) {
  ...
  if (isRevokedByIssuer) {
    throw new VerifierError(Status.checkingRevokedStatus, "This certificate has been revoked by the issuer.");
  }
}
