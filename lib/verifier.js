...
export class CertificateVerifier {
   ...
   * @param {*} err 
   */
  _failed(stepCode, completionCallback, message) {
    log(`failure:${message}`);
    this.statusCallback(stepCode, message, Status.failure);
    completionCallback(stepCode, message, Status.failure);
  ...
  doAction(stepCode, action) {
    ...
    log(message);
    this.statusCallback(stepCode, message, Status.starting);
    return action();
  }
  ...
  async verifyV2() {
    ...
    let txData = await this.doAsyncAction(
      ...
      async () => {
        return bitcoinConnectors.lookForTx(transactionId, this.certificate.chain);
      }
    );
  ...
  async verify(completionCallback) {
    ...
    try {
      ...
      if (e instanceof VerifierError) {
        return this._failed(e.stepCode, completionCallback, e.message);
      }
      throw e;
    }
  }
