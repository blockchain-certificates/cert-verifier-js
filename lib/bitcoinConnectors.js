...
import {Blockchain, CertificateVersion, MinimumBlockchainExplorers, MininumConfirmations, Race, Url} from '../config/default';
...
function getBlockcypherFetcher(transactionId, chain) {
  ...
  if (chain === Blockchain.bitcoin) {
    blockCypherUrl = Url.blockCypherUrl + transactionId;
  } else {
    blockCypherUrl = Url.blockCypherTestUrl + transactionId;
  }
...
function parseBlockCypherResponse(jsonResponse) {
  if (jsonResponse.confirmations < MininumConfirmations) {
    throw new VError("Number of transaction confirmations were less than the minimum required, according to Blockcypher API");
  }
  ...
  const opReturnScript = cleanupRemoteHash(lastOutput.data_hex);
...
function parseChainSoResponse(jsonResponse) {
  if (jsonResponse.data.confirmations < MininumConfirmations) {
    throw new VError("Number of transaction confirmations were less than the minimum required, according to Chain.so API");
  }
...
export function lookForTx(transactionId, chain, certificateVersion) {
  ...
  if (MinimumBlockchainExplorers < 0 || MinimumBlockchainExplorers > BlockchainExplorers.length) {
    return Promise.reject(new VError("Invalid application configuration; check the MinimumBlockchainExplorers configuration value"));
  }
  ...
  return new Promise((resolve, reject) => {
    return Promise.properRace(promises, MinimumBlockchainExplorers).then(winners => {
      if (!winners || winners.length == 0) {
        return Promise.reject(new VError("Could not confirm the transaction. No blockchain apis returned a response. This could be because of rate limiting."));
      }
      ...
      for (var i = 1; i < winners.length; i++) {
        ...
        if (firstResponse.issuingAddress !== thisResponse.issuingAddress) {
          throw new VError("Issuing addresses returned by the blockchain APIs were different");
        }
        if (firstResponse.remoteHash !== thisResponse.remoteHash) {
          throw new VError("Remote hashes returned by the blockchain APIs were different");
        }
      }
...
Promise.properRace = function (promises, count, results = []) {
  ...
  if (promises.length < count) {
    return Promise.reject(new VError("Could not confirm the transaction"));
  }
